...
export default function ProductClient({ id, initialProduct, initialImages }: { id: string, initialProduct: any, initialImages: string[] }) {
    const [product, setProduct] = useState<any>(initialProduct);
    const [images, setImages] = useState<string[]>(initialImages);
    const [mainImage, setMainImage] = useState<string | null>(initialImages[0] || null);
...
Please share further details.`;
    ...
    const jsonLd = {
        ...
        name: product.name,
        image: images[0] || '',
        description: product.description ? (Array.isArray(product.description) ? product.description.join(' ') : product.description) : 'Elegant luxury jewellery piece by Sayoshopping.',
        sku: product.code || product.id,
        offers: {
            '@type': 'Offer',
            url: `https://sayoshopping.lk/product/${id}`,
            priceCurrency: 'LKR',
            price: product.price,
